...
<%
    ...
    String ilkDers = dbkatmani.getIlkDers(email,sonDonem);
    session.setAttribute("dersAdi", ilkDers);
    String [][] dersNotlar = dbkatmani.getOgretmenSinavNotlari(email,ilkDers);
    session.setAttribute("sinavNotlari", dersNotlar);
    int rowCount2 = dersNotlar.length;
    int colCount2 = dersNotlar[0].length;
%>
<body>
    <div class="top-panel">
        <div class="logo">
        ...
        <div class="user-panel">
            <label for="interests">Dersler:</label>
                ...
                <% for (int i = 0; i < rowCount; i++) { %>
                    ...
                    <option <%= i == 0 ? "selected" : "" %> >(<%=dersIsim[i][j]%>) <%j=j+1;%> <%=dersIsim[i][j]%></option>
                <% }} %>
                </select>
        </div>
